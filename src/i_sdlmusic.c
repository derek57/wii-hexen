...
/*
    ...
    if (subst_music_len > 0)
    {
        C_Printf("LOADED %i OGG-MUSIC SUBSTITUTIONS.\n\n",
               subst_music_len);
    }
*/
}
...
static boolean WriteWrapperTimidityConfig(char *write_path)
{
    ...
    if(usb)
	timidity_cfg_path = "usb:/apps/wiihexen/";
    else if(sd)
	timidity_cfg_path = "sd:/apps/wiihexen/";
...
//    temp_timidity_cfg = M_TempFile("timidity.cfg");
    ...
    if(usb)
	temp_timidity_cfg = "usb:/apps/wiihexen/timidity.cfg";
    else if(sd)
	temp_timidity_cfg = "sd:/apps/wiihexen/timidity.cfg";
