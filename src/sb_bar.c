...
//        V_DrawShadowedPatch(xpos + 6 - patch->width / 2, y, patch);	// NO DEFINITELY NOT ON THE WII
...
*/
    ...
    {                           // Update mana graphics based upon mana count/weapon type
        ...
        {
/*
            I_VideoBuffer[i * SCREENWIDTH + 95] = 0;			// CHANGED FOR HIRES
            I_VideoBuffer[i * SCREENWIDTH + 96] = 0;			// CHANGED FOR HIRES
            I_VideoBuffer[i * SCREENWIDTH + 97] = 0;			// CHANGED FOR HIRES
*/
	    for (j = 0; j <= hires; j++)				// ADDED FOR HIRES
	    {								// ADDED FOR HIRES
		for (k = 0; k <= hires; k++)				// ADDED FOR HIRES
		{							// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((95 << hires) + k)] = 0;			// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((96 << hires) + k)] = 0;			// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((97 << hires) + k)] = 0;			// ADDED FOR HIRES
		}							// ADDED FOR HIRES
	    }								// ADDED FOR HIRES
        }
        ...
        {
/*
            I_VideoBuffer[i * SCREENWIDTH + 103] = 0;			// CHANGED FOR HIRES
            I_VideoBuffer[i * SCREENWIDTH + 104] = 0;			// CHANGED FOR HIRES
            I_VideoBuffer[i * SCREENWIDTH + 105] = 0;			// CHANGED FOR HIRES
*/
	    for (j = 0; j <= hires; j++)				// ADDED FOR HIRES
	    {								// ADDED FOR HIRES
		for (k = 0; k <= hires; k++)				// ADDED FOR HIRES
		{							// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((103 << hires) + k)] = 0;		// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((104 << hires) + k)] = 0;		// ADDED FOR HIRES
		    I_VideoBuffer[((i << hires) + j) * SCREENWIDTH	// ADDED FOR HIRES
			    + ((105 << hires) + k)] = 0;		// ADDED FOR HIRES
		}							// ADDED FOR HIRES
	    }								// ADDED FOR HIRES
        }
...
/*
    ...
    if(ninty == 1)
    {
	V_DrawPatch(13, 170, PatchINVHBAR);
...
//	if(oldweapon != CPlayer->readyweapon || manaVialPatch1)
	{ // Update mana graphics based upon mana count/weapon type
	    ...
	    V_DrawPatch(15, 171, manaVialPatch1);
		...
		I_VideoBuffer[i * 1 + 16] = 0;		// pixel length of the hole
		I_VideoBuffer[i * 1 + 336] = 0;		// screen, so these large
	    }								// numbers "MOVE" the line
									// that has been drawn at pixel
	    V_DrawPatch(15, 175, manaVialPatch2);			// 0x0, OVER the edge of the
		...
		I_VideoBuffer[i * 1 - 48] = 0;		// this line INTO the actual
		I_VideoBuffer[i * 1 + 272] = 0;		// place where it is needed,
