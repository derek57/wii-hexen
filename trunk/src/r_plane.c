...
void R_DrawPlanes(void)
{
    ...
    int skyTexture2;
    int scrollOffset;
    int frac;					// ADDED FOR HIRES
    int fracstep = FRACUNIT >> hires;		// ADDED FOR HIRES
                ...
                for (x = pl->minx; x <= pl->maxx; x++)
                {
                    ...
                    if (dc_yl <= dc_yh)
                    {
                        ...
                        angle = (viewangle + xtoviewangle[x])
                            >> ANGLETOSKYSHIFT;
/*
                        source = R_GetColumn(skyTexture, angle + offset)	// CHANGED FOR HIRES
                            + SKYTEXTUREMIDSHIFTED + (dc_yl - centery);		// CHANGED FOR HIRES
                        source2 = R_GetColumn(skyTexture2, angle + offset2)	// CHANGED FOR HIRES
                            + SKYTEXTUREMIDSHIFTED + (dc_yl - centery);		// CHANGED FOR HIRES
*/
			source = R_GetColumn(skyTexture, angle + offset);	// CHANGED FOR HIRES
			source2 = R_GetColumn(skyTexture2, angle + offset2);	// CHANGED FOR HIRES
                        dest = ylookup[dc_yl] + columnofs[x];
			frac = SKYTEXTUREMIDSHIFTED * FRACUNIT			// ADDED FOR HIRES
				+ (dc_yl - centery) * fracstep; 		// ADDED FOR HIRES
                        do
                        {
//                            if (*source)					// CHANGED FOR HIRES
			    if (source[frac >> FRACBITS])			// CHANGED FOR HIRES
                            {
/*
                                *dest = *source++;				// CHANGED FOR HIRES
                                source2++;					// CHANGED FOR HIRES
*/
				*dest = source[frac >> FRACBITS];		// CHANGED FOR HIRES
				frac += fracstep;				// CHANGED FOR HIRES
                            }
                            else
                            {
/*
                                *dest = *source2++;				// CHANGED FOR HIRES
                                source++;					// CHANGED FOR HIRES
*/
				*dest = source2[frac >> FRACBITS];		// CHANGED FOR HIRES
				frac += fracstep;				// CHANGED FOR HIRES
                            }
                            dest += SCREENWIDTH;
                        }
